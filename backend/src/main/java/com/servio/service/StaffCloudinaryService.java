...
import com.servio.dto.StaffFileUploadResponse;
...
public class StaffCloudinaryService {
    ...
    public StaffFileUploadResponse uploadStaffFile(MultipartFile file, String documentType, Long staffId) {
        ...
        if (!contentType.startsWith("image/") && !"application/pdf".equalsIgnoreCase(contentType)) {
            throw new RuntimeException("Only images and PDF documents are supported");
        }

        String normalizedType = normalizeDocumentType(documentType);
        String folder = staffId == null
                ? "servio/staff/" + normalizedType
                : "servio/staff/" + staffId + "/" + normalizedType;
        ...
        try {
            ...
            body.add("file", new ByteArrayResource(file.getBytes()) {
                ...
                public String getFilename() {
                    return file.getOriginalFilename() != null ? file.getOriginalFilename() : "staff-file";
                }
            });
            ...
            if (response == null || response.get("secure_url") == null || response.get("public_id") == null) {
                throw new RuntimeException("Cloudinary upload did not return a file URL");
            }

            Number bytes = response.get("bytes") instanceof Number ? (Number) response.get("bytes") : file.getSize();
            return StaffFileUploadResponse.builder()
                    .url(response.get("secure_url").toString())
                    .publicId(response.get("public_id").toString())
                    .resourceType(response.get("resource_type") != null ? response.get("resource_type").toString() : "auto")
                    .originalFilename(file.getOriginalFilename())
                    .contentType(contentType)
                    .bytes(bytes.longValue())
                    .documentType(normalizedType.toUpperCase())
                    .build();
        } catch (Exception e) {
    ...
    private String normalizeDocumentType(String documentType) {
        String value = documentType == null || documentType.isBlank() ? "other" : documentType;
